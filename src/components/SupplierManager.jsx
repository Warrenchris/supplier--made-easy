...
import { Store, Plus, Star, Clock, ShieldCheck, RefreshCw, Check } from 'lucide-react';
  ...
  return (
    <div style={{ maxWidth: '1240px', margin: '0 auto', color: '#f0f3f8' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h2 style={{ fontSize: '20px', fontWeight: 700, margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Store size={22} color="#2dd4bf" /> Supplier Management Directory
          </h2>
          <p style={{ fontSize: '13px', color: '#949eb2', marginTop: '4px' }}>
            Manage supplier profiles, reliability metrics, delivery lead times, and default commercial terms.
          </p>
        </div>
        <button
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '16px' }}>
        {suppliers.map((sup) => (
          <div key={sup.id} style={{ background: '#181b24', border: '1px solid #2b303d', borderRadius: '12px', padding: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
              <div>
                <h3 style={{ fontSize: '16px', fontWeight: 700, margin: 0 }}>{sup.name}</h3>
                <div style={{ fontSize: '12px', color: '#949eb2', marginTop: '2px' }}>{sup.contact_info || 'No contact details'}</div>
              </div>
            ...
            <div style={{ fontSize: '12px', color: '#949eb2', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <ShieldCheck size={14} color="#3bb8f6" /> Default Warranty: <strong style={{ color: '#f0f3f8' }}>{sup.warranty_terms_default}</strong>
            </div>
          </div>
        ))}
      </div>
